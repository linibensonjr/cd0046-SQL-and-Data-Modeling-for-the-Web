...
def venues():
    ...
    venues = Venue.query.group_by(
        Venue.id, Venue.city, Venue.state, Venue.name
...
def show_venue(venue_id):
    ...
    # create variables to store shows and their numbers
    upcoming_shows = []
    past_shows = []
    upcoming_shows_count = 0
    past_shows_count = 0

    # Iterate through venue shows and update upcoming show list
...
def delete_venue(venue_id):
    ...
    try:
        ...
        flash(f"An error occured. Could not delete {venue_id} from Venues")
    finally:
        db.session.close()
...
def search_artists():
    ...
    search_result = Artist.query.filter(Artist.name.ilike('%'+search_artists+'%')).all()
...
def create_artist_submission():
    ...
    try:
        ...
        post_artist = Artist(name=name, city=city, state=state, phone=phone, 
        genres=genres, facebook_link=facebook_link, image_link=image_link, website_link=website_link,
        seeking_venue=seeking_venue, seeking_description=seeking_description)

        db.session.add(post_artist)
        db.session.commit()
...
def create_show_submission():
    ...
    try:
        ...
        post_show = Show(artist_id=artist_id, venue_id=venue_id, start_time=start_time)
